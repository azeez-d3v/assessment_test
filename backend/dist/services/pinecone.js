...
exports.hasDocuments = hasDocuments;
exports.upsertChunks = upsertChunks;
exports.queryByVector = queryByVector;
exports.deleteByDocId = deleteByDocId;
 ...
 */
async function queryByVector(vector, topK = 3) {
    const client = getClient();
    const index = client.index(getIndexName());
...
async function deleteByDocId(docId) {
    ...
    do {
        ...
        if (result.vectors) {
            idsToDelete.push(...result.vectors.map(v => v.id).filter((id) => !!id));
        }
...
async function listDocuments() {
    ...
    do {
        ...
        if (result.vectors) {
            allIds.push(...result.vectors.map(v => v.id).filter((id) => !!id));
        }
        paginationToken = result.pagination?.next;
    } while (paginationToken);
    if (allIds.length === 0) {
...
async function getDocumentContent(docId) {
    ...
    do {
        ...
        if (result.vectors) {
            vectorIds.push(...result.vectors.map(v => v.id).filter((id) => !!id));
        }
        paginationToken = result.pagination?.next;
    } while (paginationToken);
    ...
    for (const id of vectorIds) {
        ...
        if (record?.metadata) {
            ...
            chunks.push({
                index: chunkIndex,
                text: record.metadata.chunkText || '',
                title: record.metadata.title || docId,
            });
        }
    }
